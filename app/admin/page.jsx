...
function SeekersTab() {
  ...
  const doMatch = async (item) => {
    ...
    if (cercaRuoli.length > 0) query = query.in("ruolo", cercaRuoli);
              ...
              <div style={d.listItemHeader} onClick={() => setExpanded(expanded === item.id ? null : item.id)}>
                <div style={d.listItemLeft}>
                  <div>
                    ...
                    <span style={d.listItemMeta}>{item.nome} · cerca: <strong style={{ color:"#c8d622" }}>{item.cerca}</strong> · {new Date(item.created_at).toLocaleDateString("it-IT")}</span>
                  </div>
                </div>
                <div style={d.listItemRight}>
                  ...
                  {matchData[item.id] && (
                    <div style={d.matchResults}>
                      <p style={d.matchResultsTitle}>
                        {matchData[item.id].length === 0 ? "Nessun creativo disponibile." : `${matchData[item.id].length} creativ${matchData[item.id].length === 1 ? "o" : "i"} compatibil${matchData[item.id].length === 1 ? "e" : "i"}`}
                      </p>
